...
class VoiceAgent:
    ...
    def __init__(self):
        ...
        self.config = {
            ...
            "allow_interruptions": os.getenv("ALLOW_INTERRUPTIONS", "true").lower()
            == "true",
            "interrupt_duration": float(
                os.getenv("INTERRUPT_SPEECH_DURATION", "0.5")
            ),
            "min_endpointing": float(os.getenv("MIN_ENDPOINTING_DELAY", "0.5")),
            "max_endpointing": float(os.getenv("MAX_ENDPOINTING_DELAY", "6.0")),
        }
    ...
    async def entrypoint(self, ctx: JobContext):
        ...
        assistant = pipeline.VoicePipelineAgent(
            ...
            interrupt_speech_duration=self.config["interrupt_duration"],
            min_endpointing_delay=self.config["min_endpointing"],
            max_endpointing_delay=self.config["max_endpointing"],
        )
